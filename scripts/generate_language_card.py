...
def render_card(totals: Counter) -> str:
    ...
    for index, (language, size) in enumerate(languages):
        ...
        bar_width = max(5, round(percentage * 5.45))
        color = palette[index % len(palette)]
        rows.append(
            f'''<text x="54" y="{y}" class="language">{escape(language)}</text>
  <rect x="245" y="{y - 17}" width="545" height="14" rx="7" fill="#21262d"/>
  <rect x="245" y="{y - 17}" width="{bar_width}" height="14" rx="7" fill="{color}"/>
  <text x="846" y="{y - 2}" class="percentage">{percentage:.1f}%</text>'''
        )
  ...
  <style>
    ...
    .percentage {{ fill: #c9d1d9; font: 600 15px -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; text-anchor: end; }}
  </style>
